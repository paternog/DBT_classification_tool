...
def VizGradCAM(model, image, interpolant=0.5, plot_results=True, self_norm=True, norm_fact=1.0):
    ...
    with tf.GradientTape() as tape:
        ...
        gradient_model = Model(inputs=[model.inputs], outputs=[target_layer.output, model.outputs[0]])
    ...
    if gradients == None:
        gradients = tf.zeros(conv2d_out.shape)
    ...
    plt.rcParams["figure.dpi"] = 100 #Enlarge the plot
